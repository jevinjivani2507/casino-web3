...
contract BaccaratGame {
    ...
    function registerPlayer() public {
        ...
        require(tokenContract.approveToken(msg.sender, betAmount),"paisa nathi");
    ...
    function distributeWinningAmount() public onlyOwner{
        ...
        if(registeredPlayers.length>=5){
            ...
            playerWinningAmount[top3Players[2]] = (totalWinningAmount * 20) / 100;
        }else if(registeredPlayers.length==4){
            playerWinningAmount[top3Players[0]] = (totalWinningAmount*50)/100;
            playerWinningAmount[top3Players[1]] = (totalWinningAmount*40)/100;
        }else if(registeredPlayers.length==3){
            playerWinningAmount[top3Players[0]] = (totalWinningAmount*60)/100;
            playerWinningAmount[top3Players[1]] = (totalWinningAmount*30)/100;
    ...
    function getTop3Players() public view onlyOwner returns (address[] memory) {
        ...
        for (uint256 i = 0; i < registeredPlayers.length; i++) {
            if (playerCardSumValue[registeredPlayers[i]] > top3PlayersCardValueSum[0]) {
                top3PlayersCardValueSum[2] = top3PlayersCardValueSum[1];
                top3PlayersCardValueSum[1] = top3PlayersCardValueSum[0];
                top3PlayersCardValueSum[0] = playerCardSumValue[registeredPlayers[i]];
                top3Players[2] = top3Players[1];
                top3Players[1] = top3Players[0];
                top3Players[0] = registeredPlayers[i];
            } else if (playerCardSumValue[registeredPlayers[i]] > top3PlayersCardValueSum[1]) {
                top3PlayersCardValueSum[2] = top3PlayersCardValueSum[1];
                top3PlayersCardValueSum[1] = playerCardSumValue[registeredPlayers[i]];
                top3Players[2] = top3Players[1];
                top3Players[1] = registeredPlayers[i];
            } else if (playerCardSumValue[registeredPlayers[i]] > top3PlayersCardValueSum[2]) {
                top3PlayersCardValueSum[2] = playerCardSumValue[registeredPlayers[i]];
